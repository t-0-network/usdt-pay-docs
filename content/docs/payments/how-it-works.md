...
- **Issuer** — creates payment instructions, watches the blockchain for the customer's payment, reports a deposit's outcome (authorized or unprocessable), and settles in USDt on-chain. At onboarding the Issuer configures an `acquirerId → settlementWallet` mapping per Acquirer it serves; the wallet may belong to the Acquirer (USDt mode) or to that Acquirer's single LP (fiat mode — each Acquirer is served by exactly one LP, fixed at onboarding). The Issuer identifies the Acquirer from the `acquirerId` carried on `5 CreatePaymentInstructions` and uses this mapping to determine the `destinationAddress` for `9 SettlementSent`. This resolution is independent of any t-0 instruction by design, so t-0's on-chain destination check on `9` is a genuine cross-check, not a confirmation of its own input. The Issuer releases deposit addresses on its own after `expiresAt`; there is no Issuer-side expiry callback.
- **Liquidity Provider (LP)** — in fiat-settlement mode, receives the Issuer's USDt and, on its own initiative, settles the locked fiat amount with the Acquirer over bank rails, reporting it to t-0 through `10 FiatSettlementSent`. Each Acquirer is served by exactly one LP, fixed at onboarding; that LP feeds t-0's Order Book on its own initiative: it pushes standing quotes (`1 PublishQuote`) and refreshes its pricing by publishing new quotes alongside earlier ones; each standing quote belongs to the LP that published it. The Acquirer's LP is fixed for all fiat-mode steps of every intent (`8`, the settlement wallet, `10`); a referenced quote (`4`) selects the rate and currency for that intent, not the LP. Currency coverage is **not** pre-checked at onboarding: a local-fiat currency is serviceable only while the LP has a standing quote for it. Because that LP is the sole source of the Acquirer's quotes, a currency it is not currently quoting is declined at `3` (`QUOTE_UNAVAILABLE`), and the LP's quote liveness gates all of the Acquirer's fiat sales — a deliberate MVP simplification with no fallback LP. In the fiat leg the LP is the Acquirer's payer — it pays over bank rails and knows the Acquirer as a payee, resolving its bank account from `acquirerId` — but not its obligor: the Issuer stays liable and the Acquirer reconciles against t-0, never the LP.
...
sequenceDiagram
    ...
    ISS->>T0: PaymentReceived (§6, authorized)
...
sequenceDiagram
    ...
    CUS->>BC: USDt transfer to chosen depositAddress
    BC-->>ISS: tx observed + screening fails

    ISS->>T0: PaymentReceived (§6, unprocessable + disposition)
    T0->>ACQ: PaymentFailed (§16, disposition)
```
...
sequenceDiagram
    ...
    Note over T0,LP: standing quotes, refreshed by publishing — not per-sale

    ACQ->>T0: GetPaymentQuote (§3)
    T0-->>ACQ: quoteId, settlementAmount, fxRate, expiresAt (filled from the Order Book)

    ACQ->>T0: CreatePaymentIntent (§4, quoteId + amount)
    T0->>ISS: CreatePaymentInstructions (§5)
    ISS-->>T0: depositOptions, expiresAt
    T0-->>ACQ: paymentIntentId, depositOptions, expiresAt
    ...
    ISS->>T0: PaymentReceived (§6, authorized)
    T0->>ACQ: PaymentAuthorized (§7)
    T0->>LP: ExecuteQuote (§8, executionId → Accepted or Rejected)
...
If the QR expires before the customer pays, t-0 — the expiry authority — transitions the intent to `EXPIRED` at `expiresAt` on its own clock and sends `15` to the Acquirer. The Issuer releases its deposit addresses on its own after `expiresAt`.
...
When the Acquirer is configured for fiat settlement (e.g., COP), the USDt↔fiat rate comes from t-0's **Order Book of standing quotes**. Each LP pushes quotes into the book on its own initiative through `1 PublishQuote` — a locked `fxRate` and a validity window — and refreshes its pricing by publishing new quotes alongside earlier ones. Before a sale, the Acquirer obtains a quote through `3 GetPaymentQuote`; t-0 answers from the book with no per-sale LP interaction. A standing quote is **multi-consumable**: the same `quoteId` may be returned to any number of quote requests and referenced by any number of payment intents while the quote stands. The Acquirer passes its `amount` — denominated in USDt (`settlement`) or in fiat (`local`; refused for an on-chain-settled Acquirer) — into `4 CreatePaymentIntent`, and may reference a `quoteId` or omit it (t-0 resolves the freshest standing quote for the payment's currency). Acceptance locks the quote's rate for that intent, and the headroom rule on `4` guarantees the quote outlives the intent's QR window.
...
stateDiagram-v2
    ...
    OPEN --> AUTHORIZED: 6 authorized
    OPEN --> FAILED: 6 unprocessable
...
**FAILED** — the Issuer will not process the deposit (terminal). Entered when `6 PaymentReceived` reports an `unprocessable` outcome; the funds follow the `disposition` (returned to sender or retained by the Issuer), and t-0 sends `16 PaymentFailed` to the Acquirer.

**EXPIRED** — reservation lapsed with no valid payment (terminal). Entered on t-0's clock at `expiresAt`; the Issuer releases deposit addresses on its own.
...
- **Amount derivation.** `settlementAmount` is derived only by t-0, at its single derivation point `4 CreatePaymentIntent` (and computed the same way as the indicative value on `3`): `settlementAmount = round(localAmount / fxRate, 2 dp, half-up)` — two decimal places, round-half-up. This holds in both settlement modes. The customer pays exactly this `settlementAmount`, and `6 PaymentReceived` requires strict equality against it.
...
- **Obligation binding** — all obligations in this protocol attach at the moment t-0 *transmits* the relevant message, except `8 ExecuteQuote`, where the LP's obligation is created only on `Accepted` (a `Rejected` result leaves the intent authorized for manual handling). LP or Issuer acknowledgment timing on other messages does not affect when an obligation is created or takes effect.
- **Decline model** — synchronous endpoints (`1`, `3`, `4`, `5`) can decline in the same call, and each synchronous response is a `result` that is `oneof { success { … } | failure { reason } }` — the `failure` variant carries a canonical `reason` code (mirroring the proto; there is no separate `declined` boolean). The asynchronous `8 ExecuteQuote` also returns a `result` (`Accepted` or `Rejected`); `Rejected` leaves the intent authorized for manual handling. Before the Issuer reports payment, the Acquirer learns of failure through `15 PaymentExpired`; after `6` reports `unprocessable`, the Acquirer learns through `16 PaymentFailed`. After `7 PaymentAuthorized` has fired the Issuer has accepted the payment and is obligated to settle it; any on-chain retry or reconciliation the Issuer performs (directly in USDt, or via an LP in fiat mode) is not exposed to the Acquirer.
- **Chain names** are a fixed canonical set maintained by t-0 — full, **case-sensitive** names, identical across every endpoint carrying a `chain` field. (The proto `BLOCKCHAIN_*` enum labels and the SQL `pay_blockchain` labels are internal representations mapped to these wire names.) **Live at launch:** `Ethereum`, `BSC`. `TRON` carries a value but is not accepted until it goes live in a later phase. **Announced as upcoming:** `Polygon`, `Arbitrum`, `Avalanche` — the Issuer offers no `depositOptions` on these, so a customer cannot pay on them through the flow until they go live. Extensible as the upcoming chains are enabled.
...
| `quoteId` | t-0 | t-0's identifier for one standing quote in its Order Book, minted on `1`. Returned by `3`; referenced by any number of `4`s while the quote stands; carried as data on `8`. Fiat-configured Acquirers only. |
| `paymentRef` | Acquirer | Echoed data on `7`, `15`, `16`. Not an idempotency key. |
| `idempotencyKey` | Acquirer | Idempotency key on `4`. Unique per Acquirer. |
| `paymentIntentId` | t-0 | Idempotency key on `5`, `6`, `7`, `15`, `16`. Not exposed to the LP — see `executionId`. |
...
`11 SettlementInitiated` (fiat mode) is keyed on t-0's `fiatSettlementId`; `13 SettlementCompleted` (USDt mode only) is keyed on t-0's `settlementId`; `16 PaymentFailed` is keyed on `paymentIntentId`. Neither `11` nor `13` uses a foreign role's id as its key — the `lpId` and `bankTransferRef` carried on `11` ride as data, not as the key. In USDt mode the Acquirer correlates the settlement through `13`'s `settledPaymentIntentIds` and `onChainTxHash`; in fiat mode through `11`'s `fiatSettlementId` + `bankTransferRef`, which it confirms with its own `12`.
...
| 15 | PaymentExpired | Acquirer | t-0 | [Acquirer](/docs/integration-guidance/api-reference/pay_acquirer/) |
| 16 | PaymentFailed | Acquirer | t-0 | [Acquirer](/docs/integration-guidance/api-reference/pay_acquirer/) |

The contract removed `2 WithdrawQuote` (LP → t-0) and `14 PaymentExpired` (Issuer → t-0); their numbers stay retired.

## Out of scope (MVP)

Refunds, chargebacks, disputes, manual cancel/void, card channel, onboarding endpoints, query/status/report endpoints, bilateral credit-line management, Acquirer→merchant payout, the LP's internal pricing and risk mechanics and bank-rail mechanics, Acquirer-side quote amendment or cancellation (standing quotes are immutable; the LP refreshes by publishing new quotes), and the POS↔Acquirer API.
