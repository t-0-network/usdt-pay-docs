...
| **POS** | Renders the QR from the payment URI the Issuer produced, without modification, and talks only to the Acquirer. |
...
| **Issuer** | Allocates the one-time deposit addresses, produces the payment URIs, watches the blockchain, screens the customer (KYT), reports the deposit outcome (authorized or unprocessable), and settles in USDT on-chain. In the fiat path its USDT settles to the LP's wallet. Remains the obligor for every authorized sale. |
| **Liquidity Provider (LP)** | Publishes standing FX quotes into t-0's book and refreshes them by publishing new quotes. On each authorized fiat sale where it accepts the execution it is bound to the quote's locked rate, delivers the local fiat to the Acquirer over bank rails, and reports it to t-0. It is the Acquirer's payer, not its obligor. |
...
sequenceDiagram
    ...
    LP->>T0: 1 PublishQuote (fxRate, expiresAt)
    ...
    ISS->>T0: 6 PaymentReceived (authorized)
    T0->>ACQ: 7 PaymentAuthorized
    T0->>LP: 8 ExecuteQuote (executionId → Accepted or Rejected)
...
### Standing quotes (`1 PublishQuote`)

The LP publishes standing FX quotes into t-0's book on its own initiative, each a locked `fxRate` (units of local currency per 1 USDT) and a validity window. A standing quote is immutable and reusable: any number of sales may price against it while it stands. The LP refreshes pricing by publishing new quotes alongside earlier ones. A currency the LP is not currently quoting is unavailable, since each Acquirer's quotes come from its single LP. In the worked example the LP's quote prices COP at 4,000 COP per USDT.

### Opening the payment intent (`3`, `4`, `5`)

**`3 GetPaymentQuote`.** Before a sale the Acquirer asks t-0 for a quote, passing the local currency and the local amount. t-0 answers from the book with a `quoteId`, the indicative `settlementAmount`, the `fxRate`, and the quote's expiry, with no per-sale interaction with the LP. For the 80,000 COP sale, t-0 returns 20.00 USDT at 4,000 COP per USDT.
...
**`6 PaymentReceived` and `7 PaymentAuthorized`.** The Issuer reports the deposit to t-0 (`6`) with an `outcome`: `authorized` or `unprocessable`. On `authorized`, t-0 requires the credited amount to equal the intent's `settlementAmount` exactly, then sends `7 PaymentAuthorized` to the Acquirer, which tells the merchant the sale is approved and releases the goods. From the moment t-0 transmits authorization, the Issuer is obligated to settle the sale. On `unprocessable`, the intent moves to FAILED and the Acquirer receives `16 PaymentFailed` with the `disposition` (returned to sender or retained by the Issuer).

**`8 ExecuteQuote`.** At authorization t-0 also sends the LP `8 ExecuteQuote`, minting a per-sale `executionId`. The LP returns **Accepted** or **Rejected**: Accepted creates the LP's firm per-sale obligation at the locked rate; Rejected leaves the intent authorized for manual handling (the Acquirer holds an authorized payment, but the fiat leg does not proceed through the standing-quote path). This step exists because the quote's validity (about 5 minutes) is shorter than the settlement batch cadence (within 15 minutes): executing the quote converts the locked rate into a firm LP obligation that survives until settlement runs.

### Settlement, in the normal order (`9`, then `10` to `12`)

**`9 SettlementSent` (Issuer to LP).** On its settlement batch, within about 15 minutes of authorization, the Issuer sends USDT on-chain to the LP's registered wallet and reports it through `9 SettlementSent`. t-0 verifies the transfer on-chain against the registered wallet and amount. This leg settles the Issuer's USDT obligation to the LP for the executions it covers. It is tracked separately from the intent and never changes the intent's state. A single `9` may cover executions for several Acquirers that LP serves. In the worked example the Issuer sends 20.00 USDT.
...
| `settlementAmount` | 20.00 USDT | t-0 derives `round(80,000 / 4,000)` at `4` |
| Customer payment | 20.00 USDT | exactly `settlementAmount`, on-chain; `6` enforces equality |
...
- **FAILED.** The Issuer will not process the deposit; the funds follow the `disposition`; t-0 sends `16 PaymentFailed` to the Acquirer. Terminal.
...
**Denomination.** The Acquirer opens the intent with an `amount` that is either `local` (a fiat figure plus currency) or `settlement` (a USDt figure). In the common case the merchant prices the sale in its own currency and the Acquirer passes the fiat `local`. When the Acquirer supplies a USDt `settlement` figure, t-0 derives the local amount from it at the locked rate and returns both in the response's `fiat` block. The quote's locked FX rate is the only conversion in play: t-0 applies it to derive the exact USDT the customer pays (`settlementAmount = round(localAmount / fxRate)`, two decimals, half-up), and the same rate fixes the USDT the Issuer later settles to the LP. The LP's obligation to the Acquirer is the local amount itself (the bill), not a rate-derived figure.
...
**The customer does not pay, or pays late.** If the QR window passes on t-0's clock with no valid payment, t-0 transitions the intent to EXPIRED and notifies the Acquirer (`15 PaymentExpired`), and the POS drops the QR. The Issuer releases its deposit addresses on its own after `expiresAt`. t-0 rejects a transfer that lands after expiry as `INTENT_EXPIRED`, and the Issuer refunds it to the `senderAddress`. Nothing is owed.

**The Issuer will not process the deposit.** The Issuer reports `6 PaymentReceived` with an `unprocessable` outcome and a `disposition` — `RETURNED_TO_SENDER` (the Issuer returns the deposit to the customer's `senderAddress`) or `RETAINED_BY_ISSUER` (the Issuer keeps the deposit; the customer resolves it with the Issuer out of band). t-0 transitions the intent to FAILED and sends `16 PaymentFailed` to the Acquirer with the same `disposition`, so the merchant can tell the customer what to expect.
...
| **Issuer** | Allocate one-time deposit addresses and produce the payment URIs, watch the blockchain, run KYT, report deposit outcomes (authorized or unprocessable), stand behind every authorized sale, and settle USDT on-chain to the LP's wallet. |
| **Liquidity Provider** | Publish and refresh standing FX quotes, accept or reject executions at the locked rate, deliver the local fiat to the Acquirer over bank rails on each accepted execution, and report each transfer to t-0. |
...
**Standing quote.** An LP-published FX quote (a locked rate and a validity window) held in t-0's book and reusable by any number of sales while it stands.
