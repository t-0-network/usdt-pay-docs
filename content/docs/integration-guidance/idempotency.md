...
|-----|-------------|-----------|-------|
| `idempotencyKey` | `4 CreatePaymentIntent` | Acquirer | per Acquirer |
...
| `paymentIntentId` | `5`, `6`, `7`, `15`, `16` | t-0 | global |

`paymentRef` on `4` is echoed data (returned on `7`, `15`, `16`), not the idempotency key; `idempotencyKey` is the retry identity for that call.

t-0-minted keys (`paymentIntentId`, `executionId`, `settlementId`, `fiatSettlementId`) are globally unique by construction. Every `t-0 → role` call is keyed on a t-0-minted id; t-0 never uses one role's id as the key in a message to another role.
...
A duplicate is a normal retry, not a fault. Returning an error for it breaks the contract and stalls the payment. Use the request's key (such as `idempotencyKey`) to tell a repeat apart from a genuinely new request.
...
- **At-least-once delivery** on the asynchronous endpoints (`6`, `7`, `8`, `9`, `10`, `11`, `12`, `13`, `15`, `16`). The sender retries with backoff until the receiver acknowledges.
